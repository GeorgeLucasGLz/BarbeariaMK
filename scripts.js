...
function enviarForm() {
    ...
    const bossNumber = "+558581861028"
    const name = document.querySelector('#inputName').value
    const number = document.querySelector('#inputNumber').value
    const message = document.querySelector('#inputText').value
    const msgDefault = "Olá Bem vindo a Barbearia MK do Corte. Sua mensagem será respondida o mais rápido possível."
    const msgInsta = "Aproveite e siga nosso perfil no Instagram: https://www.instagram.com/m.k_do_corte/"
...
const btnCuts = document.querySelector('#btn-cuts')
...
btnCuts.addEventListener('click', (event) => {

    event.preventDefault()
    scrollIntoElement('.cuts-section')
    mascara.style.visibility = "hidden"
